...
class Formatter {
  ...
  public formatSection<V, T extends Difference<V>>(
    title: string,
    entryType: string,
    collection: DifferenceCollection<V, T> | Difference<string>,
  ...
  public formatResourceDifference(_type: string, logicalId: string, diff: ResourceDifference) {
    ...
    this.print(
      `${this.formatPrefix(diff)} ${this.formatValue(resourceType, colors.cyan)} ${this.formatLogicalId(
        logicalId,
  ...
  public readConstructPathsFrom(templateDiff: TemplateDiff) {
    for (const [logicalId, resourceDiff] of Object.entries(templateDiff.resources)) {
      ...
      const oldPathMetadata =
        resourceDiff.oldValue && resourceDiff.oldValue.Metadata && resourceDiff.oldValue.Metadata[PATH_METADATA_KEY];
      ...
      const newPathMetadata =
        resourceDiff.newValue && resourceDiff.newValue.Metadata && resourceDiff.newValue.Metadata[PATH_METADATA_KEY];
