using Amazon.JSII.Runtime.Deputy;

#pragma warning disable CS0672,CS0809,CS1591

namespace AlibabaCloud.SDK.ROS.CDK.Vpc
{
    /// <summary>Properties for defining a `TrafficQosRule`.</summary>
    /// <remarks>
    /// See https://www.alibabacloud.com/help/ros/developer-reference/aliyun-vpc-trafficqosrule
    /// </remarks>
    [JsiiInterface(nativeType: typeof(ITrafficQosRuleProps), fullyQualifiedName: "@alicloud/ros-cdk-vpc.TrafficQosRuleProps")]
    public interface ITrafficQosRuleProps
    {
        /// <summary>Property priority: The priority of the QoS rule.</summary>
        /// <remarks>
        /// Valid values: 1 to 9000. A larger value indicates a higher priority. The priority of each QoS rule must be unique in the same QoS policy.
        /// </remarks>
        [JsiiProperty(name: "priority", typeJson: "{\"union\":{\"types\":[{\"primitive\":\"number\"},{\"fqn\":\"@alicloud/ros-cdk-core.IResolvable\"}]}}")]
        object Priority
        {
            get;
        }

        /// <summary>Property protocol: QoS rule protocol type, value: - **ALL** - **ICMP(IPv4)** - **ICMPv6(IPv6)* * - **TCP** - **UDP** - **GRE** - **SSH** - **Telnet** - **HTTP** - **HTTPS** - **MS SQL** - **Oracle** - **MySql** - **RDP** - **PostgreSQL** - **Redis**.</summary>
        [JsiiProperty(name: "protocol", typeJson: "{\"union\":{\"types\":[{\"primitive\":\"string\"},{\"fqn\":\"@alicloud/ros-cdk-core.IResolvable\"}]}}")]
        object Protocol
        {
            get;
        }

        /// <summary>Property qosId: The QoS policy ID.</summary>
        [JsiiProperty(name: "qosId", typeJson: "{\"union\":{\"types\":[{\"primitive\":\"string\"},{\"fqn\":\"@alicloud/ros-cdk-core.IResolvable\"}]}}")]
        object QosId
        {
            get;
        }

        /// <summary>Property queueId: The QoS queue ID.</summary>
        [JsiiProperty(name: "queueId", typeJson: "{\"union\":{\"types\":[{\"primitive\":\"string\"},{\"fqn\":\"@alicloud/ros-cdk-core.IResolvable\"}]}}")]
        object QueueId
        {
            get;
        }

        /// <summary>Property dstCidr: The destination IPv4 CIDR block that matches the QoS rule traffic.</summary>
        /// <remarks>
        ///     If this parameter is not supported, enter <strong>SrcIPv6Cidr</strong> or **DstIPv6Cidr * *.
        /// </remarks>
        [JsiiProperty(name: "dstCidr", typeJson: "{\"union\":{\"types\":[{\"primitive\":\"string\"},{\"fqn\":\"@alicloud/ros-cdk-core.IResolvable\"}]}}", isOptional: true)]
        [Amazon.JSII.Runtime.Deputy.JsiiOptional]
        object? DstCidr
        {
            get
            {
                return null;
            }
        }

        /// <summary>Property dstIpv6Cidr: The QoS rule traffic matches the Destination IPv6 network segment.</summary>
        /// <remarks>
        ///     If this parameter is not supported, enter <strong>SrcCidr</strong> or **DstCidr * *.
        /// </remarks>
        [JsiiProperty(name: "dstIpv6Cidr", typeJson: "{\"union\":{\"types\":[{\"primitive\":\"string\"},{\"fqn\":\"@alicloud/ros-cdk-core.IResolvable\"}]}}", isOptional: true)]
        [Amazon.JSII.Runtime.Deputy.JsiiOptional]
        object? DstIpv6Cidr
        {
            get
            {
                return null;
            }
        }

        /// <summary>Property dstPortRange: QoS rule traffic matches the destination port number range.</summary>
        /// <remarks>
        /// Value range: <strong>0</strong> to <strong>65535</strong>. If not, the value is -1. Currently, only a single port number is supported, and the start and end of the port number must be the same. The corresponding destination port number range is fixed for different protocol types. The values are as follows:
        ///
        /// <list type="bullet">
        /// <description><strong>ALL</strong>:-1/-1, not editable.</description>
        /// <description><strong>ICMP(IPv4)</strong>:-1/-1, non-editable.</description>
        /// <description><strong>ICMPv6(IPv6)</strong>:-1/-1, non-editable.</description>
        /// <description><strong>TCP</strong>:-1/-1, editable.</description>
        /// <description><strong>UDP</strong>:-1/-1, editable.</description>
        /// <description><strong>GRE</strong>:-1/-1, not editable.</description>
        /// <description><strong>SSH</strong>:22/22, not editable.</description>
        /// <description><strong>Telnet</strong>:23/23, not editable.</description>
        /// <description><strong>HTTP</strong>:80/80, non-editable.</description>
        /// <description><strong>HTTPS</strong>:443/443, which cannot be edited.</description>
        /// <description><strong>MS SQL</strong>:1443/1443, which cannot be edited.</description>
        /// <description><strong>Oracle</strong>:1521/1521, non-editable.</description>
        /// <description><strong>MySql</strong>:3306/3306, non-editable.</description>
        /// <description><strong>RDP</strong>:3389/3389, non-editable.</description>
        /// <description><strong>PostgreSQL</strong>:5432/5432, non-editable.</description>
        /// <description><strong>Redis</strong>:6379/6379, non-editable.</description>
        /// </list>
        /// </remarks>
        [JsiiProperty(name: "dstPortRange", typeJson: "{\"union\":{\"types\":[{\"primitive\":\"string\"},{\"fqn\":\"@alicloud/ros-cdk-core.IResolvable\"}]}}", isOptional: true)]
        [Amazon.JSII.Runtime.Deputy.JsiiOptional]
        object? DstPortRange
        {
            get
            {
                return null;
            }
        }

        /// <summary>Property matchDscp: The DSCP value that matches the QoS rule traffic.</summary>
        /// <remarks>
        /// Valid values: 0 to 63. If no value is matched, the value is -1.
        /// </remarks>
        [JsiiProperty(name: "matchDscp", typeJson: "{\"union\":{\"types\":[{\"primitive\":\"number\"},{\"fqn\":\"@alicloud/ros-cdk-core.IResolvable\"}]}}", isOptional: true)]
        [Amazon.JSII.Runtime.Deputy.JsiiOptional]
        object? MatchDscp
        {
            get
            {
                return null;
            }
        }

        /// <summary>Property remarkingDscp: The new DSCP value.</summary>
        /// <remarks>
        /// Valid values: 0 to 63. If you do not change the value, set the value to -1.
        /// </remarks>
        [JsiiProperty(name: "remarkingDscp", typeJson: "{\"union\":{\"types\":[{\"primitive\":\"number\"},{\"fqn\":\"@alicloud/ros-cdk-core.IResolvable\"}]}}", isOptional: true)]
        [Amazon.JSII.Runtime.Deputy.JsiiOptional]
        object? RemarkingDscp
        {
            get
            {
                return null;
            }
        }

        /// <summary>Property ruleDescription: The description of the QoS rule.</summary>
        /// <remarks>
        /// The length is 0 to 256 characters and cannot start with 'http:// 'or 'https.
        /// </remarks>
        [JsiiProperty(name: "ruleDescription", typeJson: "{\"union\":{\"types\":[{\"primitive\":\"string\"},{\"fqn\":\"@alicloud/ros-cdk-core.IResolvable\"}]}}", isOptional: true)]
        [Amazon.JSII.Runtime.Deputy.JsiiOptional]
        object? RuleDescription
        {
            get
            {
                return null;
            }
        }

        /// <summary>Property ruleName: The name of the QoS rule.</summary>
        /// <remarks>
        /// The length is 0 to 128 characters and cannot start with 'http:// 'or 'https.
        /// </remarks>
        [JsiiProperty(name: "ruleName", typeJson: "{\"union\":{\"types\":[{\"primitive\":\"string\"},{\"fqn\":\"@alicloud/ros-cdk-core.IResolvable\"}]}}", isOptional: true)]
        [Amazon.JSII.Runtime.Deputy.JsiiOptional]
        object? RuleName
        {
            get
            {
                return null;
            }
        }

        /// <summary>Property srcCidr: The source IPv4 CIDR block that matches the QoS rule traffic.</summary>
        /// <remarks>
        ///     If this parameter is not supported, enter <strong>SrcIPv6Cidr</strong> or **DstIPv6Cidr * *.
        /// </remarks>
        [JsiiProperty(name: "srcCidr", typeJson: "{\"union\":{\"types\":[{\"primitive\":\"string\"},{\"fqn\":\"@alicloud/ros-cdk-core.IResolvable\"}]}}", isOptional: true)]
        [Amazon.JSII.Runtime.Deputy.JsiiOptional]
        object? SrcCidr
        {
            get
            {
                return null;
            }
        }

        /// <summary>Property srcIpv6Cidr: The QoS rule traffic matches the source IPv6 network segment.</summary>
        /// <remarks>
        ///     If this parameter is not supported, enter <strong>SrcCidr</strong> or **DstCidr * *.
        /// </remarks>
        [JsiiProperty(name: "srcIpv6Cidr", typeJson: "{\"union\":{\"types\":[{\"primitive\":\"string\"},{\"fqn\":\"@alicloud/ros-cdk-core.IResolvable\"}]}}", isOptional: true)]
        [Amazon.JSII.Runtime.Deputy.JsiiOptional]
        object? SrcIpv6Cidr
        {
            get
            {
                return null;
            }
        }

        /// <summary>Property srcPortRange: The source port number of the QoS rule traffic matching.</summary>
        /// <remarks>
        /// The value range is <strong>0</strong> to <strong>65535</strong>. If the traffic does not match, the value is -1. Currently, only a single port number is supported, and the start and end of the port number must be the same.
        /// </remarks>
        [JsiiProperty(name: "srcPortRange", typeJson: "{\"union\":{\"types\":[{\"primitive\":\"string\"},{\"fqn\":\"@alicloud/ros-cdk-core.IResolvable\"}]}}", isOptional: true)]
        [Amazon.JSII.Runtime.Deputy.JsiiOptional]
        object? SrcPortRange
        {
            get
            {
                return null;
            }
        }

        /// <summary>Properties for defining a `TrafficQosRule`.</summary>
        /// <remarks>
        /// See https://www.alibabacloud.com/help/ros/developer-reference/aliyun-vpc-trafficqosrule
        /// </remarks>
        [JsiiTypeProxy(nativeType: typeof(ITrafficQosRuleProps), fullyQualifiedName: "@alicloud/ros-cdk-vpc.TrafficQosRuleProps")]
        internal sealed class _Proxy : DeputyBase, AlibabaCloud.SDK.ROS.CDK.Vpc.ITrafficQosRuleProps
        {
            private _Proxy(ByRefValue reference): base(reference)
            {
            }

            /// <summary>Property priority: The priority of the QoS rule.</summary>
            /// <remarks>
            /// Valid values: 1 to 9000. A larger value indicates a higher priority. The priority of each QoS rule must be unique in the same QoS policy.
            /// </remarks>
            [JsiiProperty(name: "priority", typeJson: "{\"union\":{\"types\":[{\"primitive\":\"number\"},{\"fqn\":\"@alicloud/ros-cdk-core.IResolvable\"}]}}")]
            public object Priority
            {
                get => GetInstanceProperty<object>()!;
            }

            /// <summary>Property protocol: QoS rule protocol type, value: - **ALL** - **ICMP(IPv4)** - **ICMPv6(IPv6)* * - **TCP** - **UDP** - **GRE** - **SSH** - **Telnet** - **HTTP** - **HTTPS** - **MS SQL** - **Oracle** - **MySql** - **RDP** - **PostgreSQL** - **Redis**.</summary>
            [JsiiProperty(name: "protocol", typeJson: "{\"union\":{\"types\":[{\"primitive\":\"string\"},{\"fqn\":\"@alicloud/ros-cdk-core.IResolvable\"}]}}")]
            public object Protocol
            {
                get => GetInstanceProperty<object>()!;
            }

            /// <summary>Property qosId: The QoS policy ID.</summary>
            [JsiiProperty(name: "qosId", typeJson: "{\"union\":{\"types\":[{\"primitive\":\"string\"},{\"fqn\":\"@alicloud/ros-cdk-core.IResolvable\"}]}}")]
            public object QosId
            {
                get => GetInstanceProperty<object>()!;
            }

            /// <summary>Property queueId: The QoS queue ID.</summary>
            [JsiiProperty(name: "queueId", typeJson: "{\"union\":{\"types\":[{\"primitive\":\"string\"},{\"fqn\":\"@alicloud/ros-cdk-core.IResolvable\"}]}}")]
            public object QueueId
            {
                get => GetInstanceProperty<object>()!;
            }

            /// <summary>Property dstCidr: The destination IPv4 CIDR block that matches the QoS rule traffic.</summary>
            /// <remarks>
            ///     If this parameter is not supported, enter <strong>SrcIPv6Cidr</strong> or **DstIPv6Cidr * *.
            /// </remarks>
            [JsiiOptional]
            [JsiiProperty(name: "dstCidr", typeJson: "{\"union\":{\"types\":[{\"primitive\":\"string\"},{\"fqn\":\"@alicloud/ros-cdk-core.IResolvable\"}]}}", isOptional: true)]
            public object? DstCidr
            {
                get => GetInstanceProperty<object?>();
            }

            /// <summary>Property dstIpv6Cidr: The QoS rule traffic matches the Destination IPv6 network segment.</summary>
            /// <remarks>
            ///     If this parameter is not supported, enter <strong>SrcCidr</strong> or **DstCidr * *.
            /// </remarks>
            [JsiiOptional]
            [JsiiProperty(name: "dstIpv6Cidr", typeJson: "{\"union\":{\"types\":[{\"primitive\":\"string\"},{\"fqn\":\"@alicloud/ros-cdk-core.IResolvable\"}]}}", isOptional: true)]
            public object? DstIpv6Cidr
            {
                get => GetInstanceProperty<object?>();
            }

            /// <summary>Property dstPortRange: QoS rule traffic matches the destination port number range.</summary>
            /// <remarks>
            /// Value range: <strong>0</strong> to <strong>65535</strong>. If not, the value is -1. Currently, only a single port number is supported, and the start and end of the port number must be the same. The corresponding destination port number range is fixed for different protocol types. The values are as follows:
            ///
            /// <list type="bullet">
            /// <description><strong>ALL</strong>:-1/-1, not editable.</description>
            /// <description><strong>ICMP(IPv4)</strong>:-1/-1, non-editable.</description>
            /// <description><strong>ICMPv6(IPv6)</strong>:-1/-1, non-editable.</description>
            /// <description><strong>TCP</strong>:-1/-1, editable.</description>
            /// <description><strong>UDP</strong>:-1/-1, editable.</description>
            /// <description><strong>GRE</strong>:-1/-1, not editable.</description>
            /// <description><strong>SSH</strong>:22/22, not editable.</description>
            /// <description><strong>Telnet</strong>:23/23, not editable.</description>
            /// <description><strong>HTTP</strong>:80/80, non-editable.</description>
            /// <description><strong>HTTPS</strong>:443/443, which cannot be edited.</description>
            /// <description><strong>MS SQL</strong>:1443/1443, which cannot be edited.</description>
            /// <description><strong>Oracle</strong>:1521/1521, non-editable.</description>
            /// <description><strong>MySql</strong>:3306/3306, non-editable.</description>
            /// <description><strong>RDP</strong>:3389/3389, non-editable.</description>
            /// <description><strong>PostgreSQL</strong>:5432/5432, non-editable.</description>
            /// <description><strong>Redis</strong>:6379/6379, non-editable.</description>
            /// </list>
            /// </remarks>
            [JsiiOptional]
            [JsiiProperty(name: "dstPortRange", typeJson: "{\"union\":{\"types\":[{\"primitive\":\"string\"},{\"fqn\":\"@alicloud/ros-cdk-core.IResolvable\"}]}}", isOptional: true)]
            public object? DstPortRange
            {
                get => GetInstanceProperty<object?>();
            }

            /// <summary>Property matchDscp: The DSCP value that matches the QoS rule traffic.</summary>
            /// <remarks>
            /// Valid values: 0 to 63. If no value is matched, the value is -1.
            /// </remarks>
            [JsiiOptional]
            [JsiiProperty(name: "matchDscp", typeJson: "{\"union\":{\"types\":[{\"primitive\":\"number\"},{\"fqn\":\"@alicloud/ros-cdk-core.IResolvable\"}]}}", isOptional: true)]
            public object? MatchDscp
            {
                get => GetInstanceProperty<object?>();
            }

            /// <summary>Property remarkingDscp: The new DSCP value.</summary>
            /// <remarks>
            /// Valid values: 0 to 63. If you do not change the value, set the value to -1.
            /// </remarks>
            [JsiiOptional]
            [JsiiProperty(name: "remarkingDscp", typeJson: "{\"union\":{\"types\":[{\"primitive\":\"number\"},{\"fqn\":\"@alicloud/ros-cdk-core.IResolvable\"}]}}", isOptional: true)]
            public object? RemarkingDscp
            {
                get => GetInstanceProperty<object?>();
            }

            /// <summary>Property ruleDescription: The description of the QoS rule.</summary>
            /// <remarks>
            /// The length is 0 to 256 characters and cannot start with 'http:// 'or 'https.
            /// </remarks>
            [JsiiOptional]
            [JsiiProperty(name: "ruleDescription", typeJson: "{\"union\":{\"types\":[{\"primitive\":\"string\"},{\"fqn\":\"@alicloud/ros-cdk-core.IResolvable\"}]}}", isOptional: true)]
            public object? RuleDescription
            {
                get => GetInstanceProperty<object?>();
            }

            /// <summary>Property ruleName: The name of the QoS rule.</summary>
            /// <remarks>
            /// The length is 0 to 128 characters and cannot start with 'http:// 'or 'https.
            /// </remarks>
            [JsiiOptional]
            [JsiiProperty(name: "ruleName", typeJson: "{\"union\":{\"types\":[{\"primitive\":\"string\"},{\"fqn\":\"@alicloud/ros-cdk-core.IResolvable\"}]}}", isOptional: true)]
            public object? RuleName
            {
                get => GetInstanceProperty<object?>();
            }

            /// <summary>Property srcCidr: The source IPv4 CIDR block that matches the QoS rule traffic.</summary>
            /// <remarks>
            ///     If this parameter is not supported, enter <strong>SrcIPv6Cidr</strong> or **DstIPv6Cidr * *.
            /// </remarks>
            [JsiiOptional]
            [JsiiProperty(name: "srcCidr", typeJson: "{\"union\":{\"types\":[{\"primitive\":\"string\"},{\"fqn\":\"@alicloud/ros-cdk-core.IResolvable\"}]}}", isOptional: true)]
            public object? SrcCidr
            {
                get => GetInstanceProperty<object?>();
            }

            /// <summary>Property srcIpv6Cidr: The QoS rule traffic matches the source IPv6 network segment.</summary>
            /// <remarks>
            ///     If this parameter is not supported, enter <strong>SrcCidr</strong> or **DstCidr * *.
            /// </remarks>
            [JsiiOptional]
            [JsiiProperty(name: "srcIpv6Cidr", typeJson: "{\"union\":{\"types\":[{\"primitive\":\"string\"},{\"fqn\":\"@alicloud/ros-cdk-core.IResolvable\"}]}}", isOptional: true)]
            public object? SrcIpv6Cidr
            {
                get => GetInstanceProperty<object?>();
            }

            /// <summary>Property srcPortRange: The source port number of the QoS rule traffic matching.</summary>
            /// <remarks>
            /// The value range is <strong>0</strong> to <strong>65535</strong>. If the traffic does not match, the value is -1. Currently, only a single port number is supported, and the start and end of the port number must be the same.
            /// </remarks>
            [JsiiOptional]
            [JsiiProperty(name: "srcPortRange", typeJson: "{\"union\":{\"types\":[{\"primitive\":\"string\"},{\"fqn\":\"@alicloud/ros-cdk-core.IResolvable\"}]}}", isOptional: true)]
            public object? SrcPortRange
            {
                get => GetInstanceProperty<object?>();
            }
        }
    }
}
