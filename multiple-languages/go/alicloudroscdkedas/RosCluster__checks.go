...
func (j *jsiiProxy_RosCluster) validateSetLogicalRegionIdParameters(val interface{}) error {
	switch val.(type) {
	case *string:
		// ok
	case string:
...
func (j *jsiiProxy_RosCluster) validateSetNetworkModeParameters(val interface{}) error {
	if val == nil {
		return fmt.Errorf("parameter val is required, but nil was provided")
	}
	switch val.(type) {
...
func (j *jsiiProxy_RosCluster) validateSetOversoldFactorParameters(val interface{}) error {
	switch val.(type) {
	case *float64:
		// ok
	case float64:
