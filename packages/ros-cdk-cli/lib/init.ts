...
export async function cliInit(
  ...
  const template = (await availableInitTemplates).find((t) => t.hasName(appName!));
  if (!template) {
  ...
  if (!language && template.languages.length === 1) {
    ...
    warning(
      `No --language was provided, but '${appName}' supports only '${language}', so defaulting to --language=${language}`,
    );
  }
  if (!language) {
    print(
      `Available languages for ${colors.green(appName)}: ${template.languages.map((l) => colors.blue(l)).join(', ')}`,
    );
...
export const availableInitTemplates: Promise<InitTemplate[]> = new Promise(async (resolve) => {
...
export const availableInitLanguages: Promise<string[]> = new Promise(async (resolve) => {
