...
export function prepareApp(root: IConstruct) {
  if (root.node.scope && !Stage.isStage(root)) {
    throw new Error(
      "prepareApp can only be called on a stage or a root construct"
    );
  }
 ...
 */
function findRosResources(root: IConstruct): RosResource[] {
  return root.node.findAll().filter(RosResource.isRosResource);
}
