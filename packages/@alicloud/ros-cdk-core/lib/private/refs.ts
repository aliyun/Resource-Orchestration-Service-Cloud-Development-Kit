...
import { RosOutput } from "../ros-output";
...
import { RosReference } from "./ros-reference";
 ...
 */
function resolveValue(consumer: Stack, reference: RosReference): IResolvable {
...
function findAllReferences(root: IConstruct) {
  const result = new Array<{ source: RosElement; value: RosReference }>();
  for (const consumer of root.node.findAll()) {
    // include only RosElements (i.e. resources)
    if (!RosElement.isRosElement(consumer)) {
      continue;
    }

    try {
      const tokens = findTokens(consumer, () => consumer._toRosTemplate());
      ...
      for (const token of tokens) {
        ...
        if (!RosReference.isRosReference(token)) {
          continue;
        }
  ...
  if (Token.isUnresolved(exportName)) {
    throw new Error(
      `unresolved token in generated export name: ${JSON.stringify(
        exportingStack.resolve(exportName)
      )}`
    );
  }

  const output = exportsScope.node.tryFindChild(id) as RosOutput;
  if (!output) {
    new RosOutput(exportsScope, id, {
...
function createNestedStackParameter(
  ...
  return param.value as RosReference;
}
...
function createNestedStackOutput(
  ...
  let output = producer.node.tryFindChild(outputId) as RosOutput;
  if (!output) {
    output = new RosOutput(producer, outputId, {
      value: Token.asString(reference),
    });
  }
