...
import {
  ...
  pathToTopLevelStack as pathToRoot,
} from "./util";
 ...
 * diagnostics
 */
export function addDependency<T extends Element>(
  source: T,
  target: T,
  reason?: string
) {
  ...
  if (sourceStage !== targetStage) {
    throw new Error(
      `You cannot add a dependency from '${
        source.node.path
      }' (in ${describeStage(sourceStage)}) to '${
        target.node.path
      }' (in ${describeStage(
        targetStage
  ...
  if (commonStack === target) {
    throw new Error(
      `Nested stack '${sourceStack.node.path}' cannot depend on a parent stack '${targetStack.node.path}': ${reason}`
    );
  }
  ...
  function resourceInCommonStackFor(element: RosResource | Stack): RosResource {
    const resource = Stack.isStack(element)
