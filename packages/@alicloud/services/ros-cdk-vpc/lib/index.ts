export * from './anycasteip';
export * from './anycasteipassociation';
export * from './bgpgroup';
export * from './bgpnetwork';
export * from './bgppeer';
export * from './commonbandwidthpackage';
export * from './commonbandwidthpackageip';
export * from './customergateway';
export * from './dhcpoptionsset';
export * from './dhcpoptionssetattachment';
export * from './eip';
export * from './eipassociation';
export * from './eippro';
export * from './eipsegment';
export * from './flowlog';
export * from './forwardentry';
export * from './fullnatentry';
export * from './grantinstancetocen';
export * from './havip';
export * from './havipassociation';
export * from './ipam';
export * from './ipampool';
export * from './ipamresourcediscovery';
export * from './ipamscope';
export * from './ipsecserver';
export * from './ipv4gateway';
export * from './ipv6egressrule';
export * from './ipv6gateway';
export * from './ipv6internetbandwidth';
export * from './natgateway';
export * from './natip';
export * from './natipcidr';
export * from './networkacl';
export * from './networkaclassociation';
export * from './peeringrouterinterfacebinding';
export * from './peeringrouterinterfaceconnection';
export * from './prefixlist';
export * from './publicipaddresspool';
export * from './routetable';
export * from './routetableassociation';
export * from './routetableattachment';
export * from './routetablewithgatewayassociation';
export * from './routerinterface';
export * from './snatentry';
export * from './sslvpnclientcert';
export * from './sslvpnserver';
export * from './trafficmirrorfilter';
export * from './trafficmirrorsession';
export * from './trafficmirrorsessionsourcesaddition';
export * from './trafficqos';
export * from './trafficqosqueue';
export * from './trafficqosrule';
export * from './vswitchcidrreservation';
export * from './vcorouteentry';
export * from './virtualborderrouter';
export * from './vpccidrblockassociation';
export * from './vpcgatewayendpoint';
export * from './vpcgatewayendpointassociation';
export * from './vpcipv6cidrallocation';
export * from './vpcpeerconnection';
export * from './vpnattachment';
export * from './vpnconnection';
export * from './vpngateway';
export * from './vpnpbrrouteentry';
export * from './vpnrouteentry';

// ALIYUN::VPC Resources:
export * from './vpc.generated';
export * as datasource from './datasource';
