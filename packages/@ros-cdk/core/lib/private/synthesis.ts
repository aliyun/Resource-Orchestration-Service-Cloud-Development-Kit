...
function invokeAspects(root: IConstruct) {
  ...
  function recurse(
    ...
    inheritedAspects: constructs.IAspect[]
  ) {
    // hackery to be able to access some private members with strong types (yack!)
    const node: NodeWithAspectPrivatesHangingOut = construct.node
      ._actualNode as any;
...
function visit(
  root: IConstruct,
  order: "pre" | "post",
  cb: (x: IProtectedConstructMethods) => void
) {
  if (order === "pre") {
 ...
 * Hackery!
 */
type NodeWithAspectPrivatesHangingOut = Omit<
  constructs.Node,
  "invokedAspects" | "_aspects"
> & {
